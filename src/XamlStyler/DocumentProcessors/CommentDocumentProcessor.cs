    ...
    internal class CommentDocumentProcessor : IDocumentProcessor
    {
        ...
        public void Process(XmlReader xmlReader, StringBuilder output, ElementProcessContext elementProcessContext)
        {
            ...
            if (content.Contains("<") && content.Contains(">"))
            {
                ...
                if (content.Contains("\n"))
                {
                    output.Append(String.Join(elementProcessContext.NewLine, content.GetLines().Select(_ => _.TrimEnd(' '))));
            ...
            else if (content.Contains("\n"))
            {
                ...
                {
                    output.Append(elementProcessContext.NewLine).Append(contentIndentString).Append(line.Trim());
                }

                output.Append(elementProcessContext.NewLine).Append(currentIndentString).Append("-->");
            }
            else
            {
                output
